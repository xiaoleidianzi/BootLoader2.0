...
extern unsigned short BootStatus ;		//状态码表
extern unsigned int Software_Ver ;		//数据包版本号
extern unsigned int Compiler_Date;		//数据包生产日期
extern _Bool UPSTART;
extern char Pkg_Count;
/*
************************************************************
*	函数名称：	OTA_UrlEncode
*
...
_Bool OneNET_RegisterDevice(void)
{
	...
	*/
	...
	data_ptr = (char *)ESP8266_MQTTIPD(250);							//等待平台响应
...
_Bool OneNet_DevLink(void)
{
	...
	OneNET_Authorization("2018-10-31", PROID, 1956499200, DEVICE_KEY, DEVICE_NAME,
								authorization_buf, sizeof(authorization_buf), 0);
	...
	if(MQTT_PacketConnect(PROID, authorization_buf, DEVICE_NAME, 256, 1, MQTT_QOS_LEVEL0, NULL, NULL, 0, &mqttPacket) == 0)
	{
		...
		dataPtr = ESP8266_MQTTIPD(250);									//等待平台响应
		if(dataPtr != NULL)
		{
...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	strcpy(buf, "{\"id\":123,\"dp\":{");//	\"代表一个双引号字符
	
//	memset(text, 0, sizeof(text));
//	sprintf(text, "\"Step\":[{\"v\":%3f}],",temperatrue); 
//	//sprintf(text, "\"Red_Led\":[{\"v\":%d}],", led_status.Led4Sta);
//	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	//sprintf(text, "\"power\":%3f",power); 
	sprintf(text, "\"Status\":[{\"v\":\"%x\"}],",BootStatus); //16进制数据需要双引号括起来
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"SoftwareVer\":[{\"v\":\"%x\"}],", Software_Ver);
	strcat(buf, text);
//	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"CompilerDate\":[{\"v\":\"%x\"}],", Compiler_Date);
	strcat(buf, text);
//	
//	memset(text, 0, sizeof(text));
//	sprintf(text, "\"LedF8\":[{\"v\":%d}],", led_status.LedF8Sta);
//	strcat(buf, text);
//	
//	memset(text, 0, sizeof(text));
//	sprintf(text, "\"LedF9\":[{\"v\":%d}]", led_status.LedF9Sta);
//	strcat(buf, text);
...
void OneNet_RevPro(unsigned char *cmd)
{
	...
	if(dataPtr != NULL && result != -1)					//如果找到了
	{
		...
		if(strstr((char *)req_payload, "UPSTART"))		//搜索"redled"
		{
			if(num == 1)								
			{
				 UPSTART = 1;							//收到开始升级指令
				 //Pkg_Count = num;					//升级包总包数
			}
			else 					
			{
				 UPSTART = 0;							//未收到开始升级指令
		...
		else if(strstr((char *)req_payload, "LedF6"))		//搜索"redled"
		{
			...
			else if(num == 0)							//控制数据如果为0，代表关
			{
				LedF6_Set(LED_OFF);
														...
														//下同
		else if(strstr((char *)req_payload, "LedF7"))
		{
			if(num == 1)
			{
				LedF7_Set(LED_ON);
		...
		else if(strstr((char *)req_payload, "LedF8"))
		{
			if(num == 1)
			{
				LedF8_Set(LED_ON);
		...
		else if(strstr((char *)req_payload, "LedF9"))
		{
			if(num == 1)
			{
				LedF9_Set(LED_ON);
